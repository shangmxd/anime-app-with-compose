...
import com.example.animeapp.utils.toAnime
...
class MainRepository @Inject constructor(
    ...
    fun getToWatchAnimeIDs(): Flow<List<Int>> =
        animeDao.getAllToWatchAnime()
            .map {listOfToWatch ->
                listOfToWatch.map {
                    it.animeId
                }
            }
    fun getToWatchAnimeIDsInAnimeModel(): Flow<List<Anime>> =
        animeDao.getAllToWatchAnime()
            .map { listOfToWatchEntity ->
                listOfToWatchEntity.map(ToWatchEntity::toAnime)
            }


    suspend fun addAnimeToFav(animeId: Int) =
        animeDao.addAnimeToFavourites(
            FavouritesEntity(
                animeID = animeId
            )
        )
    ...
    suspend fun deleteAnimeFromFav(animeId: Int) =
        animeDao.deleteAnimeFromFavourites(
            FavouritesEntity(
                animeID = animeId
            )
        )
    ...
    suspend fun addAnimeToWatchlist(anime: Anime) =
        animeDao.addAnimeToWatch(
            ToWatchEntity(
                ...
                title = anime.title,
                year = anime.year,
                backgroundImg = anime.images.jpg.imageUrl
            )
        )
